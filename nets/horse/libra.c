...
void create()
{
set_name(HIC"傲雪狂龙"NOR, ({ "crazy dragon","dragon" }) );
...
int do_ascend()
{
   ...
   if (name != "libra" && name != "libraa")
           return notify_fail("你不是这座骑的主人(Libra)无法骑他!!!\n");

        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的仙龙身上呀。\n");

message_vision("$N跳上了傲云狂龙,云雾把$N的身体包覆起来.使你产生了无比的信心。\n",me);
...
message_vision("$N跳下云傲狂龙使云傲狂龙化为一团云雾...\n",me);
...
{
      message_vision(HIW+"道道闪光从傲雪狂龙口中疾吐而出,正朝着$N迅速直劈而去!!\n"+NOR,enemy[i]);
  ...
  COMBAT_D->report_status(enemy[i], 1);
      ...
      message_vision(HIW+"道道闪光傲雪狂龙的口中疾吐而出,正朝着$N迅速直袭而去!!\n"+NOR,enemy[i]);
