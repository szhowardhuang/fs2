...
void create()
{
set_name(HIY"人"HIG"间"HIR"凶"HIB"兽"HIC"-"HIW"天罪"NOR, ({ "sky sin","sin" }) );
...
int do_ascend()
{
        ...
        if (name != "rry")
           return notify_fail("你不是这座骑的主人(rry)无法骑他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的座骑身上呀。\n");

        message_vision("$N狂啸一声只见一道雪白的光影飞至身前，$N跨上了"+this_object()->name()+"。\n",me);
...
message_vision("$N一个前空翻从"+this_object()->name()+"的背上跳了下来。\n",me);
