...
void create()
{
set_name(HIY"诸神的黄昏"NOR, ({ "god horse","horse" }) );
...
int do_ascend()
{
        ...
        if (name != "snowcraft")
           return notify_fail("你不是这座骑的主人(snowcraft)无法骑他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的仙龙身上呀。\n");

message_vision(""HIY"$N口中默念几句咒语，突然间狂风大作，金光暴现，"+this_object()->name()+""HIY"在\n"
               "天空中幻化成一匹金黄色的巨兽。\n"NOR"",me);
...
message_vision(""HIY"$N随手将背上的剑翘往天空中一丢，巨兽随即往剑翘冲去，\n"
               "煞时间天空中闪过一道金色光芒，只见"+this_object()->name()+"随着剑翘巧\n"
               "妙的落回$N背上。\n"NOR"",me);
