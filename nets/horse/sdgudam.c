...
void create()
{
set_name(HIC"啸天犬"NOR, ({ "fly dog","dog" }) );
set("long","这是生存在仙界九天之上的仙犬,是巫师们赏赐给人们的\n似乎还能骑上去唷(ride)");
set("no_drop", 1);
set("no_auc", 1);
set("no_sell", 1);
set("unit","只");
...
int do_ascend()
{
        ...
        if (name != "sdgudam")
           return notify_fail("你不是这座骑的主人(sdgudam)无法骑他!!!\n");

        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的座骑身上呀。\n");

        message_vision("$N仰天长啸跳向天空,顿时天空一声怒吼,只见$N已跨在"+this_object()->name()+"的身上。\n",me);
...
message_vision("$N一个后空翻从"+this_object()->name()+"的背上跳了下来,瞬间"+this_object()->name()+"消失无踪。\n",me);
