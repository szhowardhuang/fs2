...
void create()
{
set_name(RED"八焚"HIW"杀"NOR""MAG"魔"NOR, ({ "sky-kill","kill" }) );
...
int do_ascend()
{
        ...
        if (name != "hikki")
           return notify_fail("你不是这座骑的主人(hikki)无法骑他!!!\n");
...
message_vision("$N仰天狂笑，呼啸疾过，只见"+this_object()->name()+"从天彩神天中缓缓降下。\n",me);
...
message_vision("$N脚踏神驹之首，从"+this_object()->name()+"身上飞奔而下，好不潇洒。\n",me);
