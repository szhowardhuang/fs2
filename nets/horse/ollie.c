...
void create()
{
set_name(HIR"赤血"HIY"妖龙"NOR, ({ "blood dragon","dragon" }) );
...
int do_ascend()
{
        ...
        if (name != "ollie")
           return notify_fail("你不是这座骑的主人(ollie)无法骑他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的座骑身上呀。\n");

        message_vision("$N大喝一声只见一道血红的光影飞至，$N迅速往"+this_object()->name()+"一跨骑了上去。\n",me);
...
message_vision("$N一个后旋翻从"+this_object()->name()+"的背上摔了下来。\n",me);
