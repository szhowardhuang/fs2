...
void create()
{
set_name(HIR"超级坏马"NOR, ({ "superbad-horse","horse" }) );
...
int do_ascend()
{
        ...
        if (name != "ddt")
           return notify_fail("你不是这座骑的主人(ddt)无法骑他!!!\n");
...
message_vision("在庄家一阵叫骂之中,"+this_object()->name()+"从赌场里冲出来\n$N一个翻身落在"+this_object()->name()+"的背上。\n",me);
...
message_vision("$N纵身一跳,"+this_object()->name()+"已然消失无踪。\n",me);
