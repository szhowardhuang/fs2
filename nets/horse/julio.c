...
void create()
{
set_name(HIW"天地云龙"NOR, ({ "cloudy dragon","dragon" }) );
set("long",""HIW"他是欧胡爷爷的坐骑，看起来十分可爱逗趣，不过他可是非常残\n"
           "暴凶恶的喔!!!!!"NOR"似乎还能骑上去唷(ride)\n");
set("no_drop", 1);
set("no_auc", 1);
set("no_sell", 1);
set("unit","只");
...
int do_ascend()
{
        ...
        if (name != "julio")
           return notify_fail("你不是这座骑的主人(julio)无法骑他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的仙龙身上呀。\n");

message_vision(""HIC"$N向云端招了招手，一声怒吼，狂暴的"+this_object()->name()+"自天际狂奔而出。\n"NOR"",me);
...
message_vision(""HIG"$N随手一挥，狂暴的"+this_object()->name()+"再度变为云雾而消散于天地之间。\n"NOR"",me);
