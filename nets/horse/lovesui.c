...
void create()
{
set_name(BLU"风残"HIW"雪影"NOR, ({ "shadow snow","snow" }) );
...
int do_ascend()
{
        ...
        if (name != "lovesui")
           return notify_fail("你不是这座骑的主人(lovesui)无法骑他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的仙龙身上呀。\n");

message_vision("只见$N一瞬间消失于你眼前,以迅雷不及掩耳的速度,$N已驾驭在"+this_object()->name()+"的背上。\n",me);
...
message_vision("$N一个翻身,口中喃喃地念着咒语,"+this_object()->name()+"竟渐渐散化于空气中。\n",me);
