...
void create()
{
set_name(HIC"圣兽"HIR"牙炎"NOR, ({ "god dragon ","dragon" }) );
set("long","传说中操纵火焰的圣兽，只有他所承认的继承人可以召唤他，\n"
           "拥有不可估计的神秘力量，似乎还能骑上去唷(ride)\n");
set("no_drop", 1);
set("no_auc", 1);
set("no_sell", 1);
set("unit","只");
...
int do_ascend()
{
        ...
        if (name != "kaze")
           return notify_fail("你不是这座骑的主人(kaze)无法骑他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的仙龙身上呀。\n");

message_vision("在火焰的魔法阵中，"+this_object()->name()+"伴随着熊熊烈火降临。$N身形一晃\n"
               "立刻出现在"+this_object()->name()+"的身上，全身充满非凡的气势。\n",me);
...
message_vision(""+this_object()->name()+"在一声清啸后消失在次元的夹缝中，现场只留下熊熊的天火。\n",me);
