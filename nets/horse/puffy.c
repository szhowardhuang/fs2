...
void create()
{
set_name(HIW"的卢马"NOR, ({ "de horse","horse" }) );
set("long","此马是三国时代赵云斩了盗贼守领后所得。\n"
           "似乎还能骑上去唷(ride)\n");
set("no_drop", 1);
set("no_auc", 1);
set("no_sell", 1);
...
int do_ascend()
{
        ...
        if (name != "puffy")
           return notify_fail("你不是这座骑的主人(puffy)无法骑他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你无法骑在别人的仙龙身上呀。\n");

message_vision("$N翻身上马，"+this_object()->name()+"感受到你的威势而活跃起来。\n",me);
...
message_vision("$N一个鹞子翻身，帅气的从"+this_object()->name()+"的背上翻落地面。\n",me);
