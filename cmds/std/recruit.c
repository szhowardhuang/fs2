// recruit.c
// Modify By Oda  把叛师的处罚放到 /feature/apprentice.c 里面, 以免拜 npc
//                时处罚不到.
...
return notify_fail("马不能收徒。\n");
	...
	if( ob->query("人际关系/"+me->query("id")) < LEAST_RELATIONSHIP )
		return notify_fail(ob->name()+"跟你的关系不够好，你不能够收他为徒。\n");
	}
