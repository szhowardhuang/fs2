...
void create()
{
   ...
   set_name(HIR"红莲匕首"NOR,({ "Red Fire Dagger","red_fire","red"}) );
   set_weight(2000);
   if( clonep() )
   ...
   {
     set("long",@LONG
	这把匕首是当年由红莲创教教主爱玫向天神所求来的，
			且只有历代教主才可以传承拥有并发挥威力；
	传说于每次战斗中，都能不断吸取万物的元气以增加本身的威力
     下列是你可以下的命令，详细内容请打help red_fire
     锻炼(discipline)     格式 discipline           ：以自身力量使光系力量成长
     吸收(absorb)         格式 absorb <target>      ：吸收对方使暗黑系力量成长
     命名(rename)         格式 rename <name>        ：为匕首改名
     资料(dag_data)       格式 dag_data             ：察看匕首目前的水准
LONG);
     set("unit","把");
     set("value",10000);
     set("material", "steel");
   set("wield_msg", HIR"$N掌心泛发出一股血红色光芒，瞬时握着传说中已失传的--$n!!\n"NOR);
   set("unwield_msg", HIR"$N大喝一声！$n瞬间幻化成一阵红烟，迅速窜入$N的掌心之内。"NOR);
   set("no_sell",1);
   set("no_drop",1);
   set("no_auc",1);
   set("no_steal",1);
   ...
   init_dagger(50);
   //更新原有的攻击力,暂定最高攻击力 lv/2 +50 +1 =100+50+1=151
...
int target_absorb(string str)
{
        ...
        message_vision(HIM"$N扬起"+name()+HIM"杀气凝聚于"+name()+HIM"幻出紫气 \n"NOR,me);
        ...
        if( random(exp2) > random(exp1)/2 )
       {
        ...
        //读出目前的暗系力量
           ...
           if (expm >= (daglv * 400))  //升级
           {
                ...
                me->set("dag/dark/power",1);
                         message_vision(HIM"$N手中的"+name()+HIM"发出紫色的光辉?
C\n"+
                "看来威力更上一层楼了\n"NOR,me);
                this_object()->delete("weapon_prop/damage");
  ...
  if( user->is_fighting() && query("equipped") )
  {
    if( random(100) > 70 )  //使驱动机率降到 30%
    {
     ...
     {//为了扩展性，例如还可以加入火之属性与水之属性
      ...
      message_vision(name()+HBCYN"一股震动隐隐感觉$N内功造诣迅速幻化"NOR,user);
      message_vision("\n$N只觉眼前一乱本身的元气已被"+name()+"的强大能量吸尽"NOR,victim);
     ...
     case 2:
     {
      message_vision(name()+HBMAG"被战斗的气劲牵引突然光芒一现,幻化成九龙凌空于场内"NOR,user);
      ...
      message_vision(HIW"$N巧妙的躲开"+name()+"的攻击\n"NOR,victim);
  }
}
...
write(@HELP
洪莲匕首，可以随着你养育他的方法而成长，具有光明与黑暗两系的力量。
两种力量并不相容，所以当其中一方的力量增强，另一方的力量必然下降。
所以，想让天诛同时具有两种属性是不可能的。
天诛在光系的力量是以回复自身为主，暗系的力量是以伤害对手为主。
因此，光系的力量必须以自身的内力去培养，
