...
set_name(HIM"魔霸封神指"NOR,({ "Yuki-Fighter-kill","yuki"}) );
   ...
   {
     set("long",@LONG
这乃吸收天地日月的精华而酝藏在凌云山中的上古神器。此乃是 Yukigirl 所拥有，
这是她费尽千辛万苦才得到的，而且这兵器似乎也能感受到她的灵性与智慧
所以已成了她的专属武器。

     下列是你可以对他下的命令，详细内容请打help yuki_fighter_kill
     锻炼(discipline)      格式 discipline           ：以自身力量使光系力量成长
     吸收(absorb)     	格式 absorb <target>  ：吸收对方使暗黑系力量成长
     命名(rename)   	格式 rename <name> ：为武器改名
     资料(dag_data)       格式 yuki_data           ：察看武器目前的水准
LONG);
     set("unit","对");
     set("value",0);
     set("material", "steel");
     set("wield_msg", HIR"$N仰天狂笑装备起$n，煞时红光四射强大的肃杀之气垄罩在四周\n"NOR);
     set("unwield_msg", HIR"$N收起杀意慢慢的将$n脱下，周围的气劲也随之消减\n"NOR);
     set("no_sell",1);
     set("no_drop",1);
     set("no_auc",1);
   }
...
{//hit_ob的BUG，第一个传入物件代表被害者，第二个不清楚
  ...
  if( user->is_fighting() && query("equipped") )
  {
	...
	{//为了扩展性，例如还可以加入火之属性与水之属性
	 ...
	 case 2:
	 {
    	  message_vision(HBMAG"被战斗的气劲牵引突然光芒一现,幻化成九龙凌空于身！"NOR,user);
...
write(@HELP
魔霸封神指可以随着你养育他的方法而成长，具有光明与黑暗两系的力量。
两种力量并不相容，所以当其中一方的力量增强，另一方的力量必然下降。
所以，想让魔霸封神指同时具有两种属性是不可能的。
魔霸封神指在光系的力量是以回复自身为主，暗系的力量是以伤害对手为主。
因此，光系的力量必须以自身的内力去培养，
