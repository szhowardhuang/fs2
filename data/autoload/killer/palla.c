...
void create()
{
   ...
   {
     set("long",@LONG
当你眼睛被此把无名所吸引，杀气顿时从刀锋射出,令你无法直视此把无名
由无名所传来的无穷威力，似乎只有扬能够作他的主人
     下列是你可以对他下的命令，详细内容请打help yang
     锻炼(discipline)      格式 discipline           ：以自身力量使光系力量成长
     吸收(absorb)     	格式 absorb <target>  ：吸收对方使暗黑系力量成长
     命名(rename)   	格式 rename <name> ：为武器改名
     资料(dag_data)       格式 yang_data          ：察看武器目前的水准
LONG);
     set("unit","把");
     set("value",0);
     set("material", "steel");
     set("wield_msg", HIB"$N仰天狂笑装备起$n，煞时红光四射强大的肃杀之气垄罩在四周\n"NOR);
     set("unwield_msg", HIB"$N收起杀意慢慢的将$n脱下，周围的气劲也随之消减\n"NOR);
     set("no_sell",1);
     set("no_drop",1);
     set("no_auc",1);
   }
...
write(@HELP
无名可以随着你养育他的方法而成长，具有光明与黑暗两系的力量。
两种力量并不相容，所以当其中一方的力量增强，另一方的力量必然下降。
所以，想让无名同时具有两种属性是不可能的。
无名在光系的力量是以回复自身为主，暗系的力量是以伤害对手为主。
因此，光系的力量必须以自身的内力去培养，
