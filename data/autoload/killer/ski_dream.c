...
void create()
{
   ...
   set_name(HIR"天诛"NOR,({ "ski-kill","ski"}) );
   set_weight(2000);
   if( clonep() )
   ...
   {
     set("long",@LONG
   这是一把由九天真气和日月之光凝化而成的,锋利无芒,光而不透有如具有生命一般,
   武器表面还浮着一股真气写着天诛二字。
     下列是你可以对他下的命令，详细内容请打help dream_dagger
...
LONG);
     ...
     set("value",10000);
     set("material", "steel");
   set("wield_msg", HIG"$N装备$n一道九天之气和日月之光从天而降全身光芒\n"NOR);
   set("unwield_msg", HIG"$N将$n 往空中一抛化作九天之气和日月之光两道真气直窜入体内\n"NOR);
   set("no_sell",1);
   set("no_drop",1);
   set("no_auc",1);
   }
...
write(@HELP
天诛，可以随着你养育他的方法而成长，具有光明与黑暗两系的力量。
