...
void create()
{
   ...
   {
     set("long",@LONG
想不到在你面前的竟然是传说中的神兵--莫邪。由剑身上传来神秘光彩，
令你感到无限的威力，据说此把神兵在近年来已经落入gendor的手中，
为其所有，其他人并无法操作这一把武器。

     下列是你可以对他下的命令，详细内容请打help moshe
     锻炼(discipline)      格式 discipline           ：以自身力量使光系力量成长
     吸收(absorb)     	格式 absorb <target>  ：吸收对方使暗黑系力量成长
     命名(rename)   	格式 rename <name> ：为武器改名
     资料(dag_data)       格式 moshe_data           ：察看武器目前的水准
LONG);
     set("unit","把");
     set("value",0);
     set("material", "steel");
     set("wield_msg", HIM"$N无生无息地把$n由腰上剑鞘拔出，感到阵阵的悲泣。\n"NOR);
     set("unwield_msg", HIM"$N无生无息地把$n插入腰上剑鞘，恢复一般的平静。\n"NOR);
     set("no_sell",1);
     set("no_drop",1);
     set("no_auc",1);
   }
...
write(@HELP
莫邪可以随着你养育他的方法而成长，具有光明与黑暗两系的力量。
两种力量并不相容，所以当其中一方的力量增强，另一方的力量必然下降。
所以，想让莫邪同时具有两种属性是不可能的。
莫邪在光系的力量是以回复自身为主，暗系的力量是以伤害对手为主。
因此，光系的力量必须以自身的内力去培养，
