...
LONG);
     ...
     set("value",0);
     set("material", "steel");
   set("wield_msg", HIY"$N随手一晃，$n从天而降使$n发出今黄色的光芒.\n"NOR);
   set("unwield_msg", HIY"$N向上一抛，$n凭空消失了\n"NOR);
   set("no_sell",1);
   set("no_drop",1);
   set("no_auc",1);
   }
...
write(@HELP
琉璃，可以随着你养育他的方法而成长，具有光明与黑暗两系的力量。
两种力量并不相容，所以当其中一方的力量增强，另一方的力量必然下降。
所以，想让琉璃同时具有两种属性是不可能的。
琉璃在光系的力量是以回复自身为主，暗系的力量是以伤害对手为主。
因此，光系的力量必须以自身的内力去培养，
