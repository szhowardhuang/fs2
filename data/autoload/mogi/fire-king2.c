...
void heart_beat()
{
	...
	if( random(100) < 5 && is_fighting() ) { // Ö÷´ò busy
	...
	if( random(100) < 10 && is_fighting() ) { // Ö÷´ò force
		message_vision( HIR"
	...
	if( random(100) < 5 && is_fighting() ) { // Ö÷´òÉñ
		message_vision( HIR"
	...
	if( random(100) < 5 && is_fighting() ) { // Ö÷´òmob
		message_vision( HIR"
	...
	if( random(100) < 10 && is_fighting() ) { // Ö÷´òÆø
		message_vision( HIR"
	·ÙÌìÄ§Íõ»¯ÉíÁ¶ÓüÒµ»ğ, ÑıÆøÔÚÒ»Ë²¼ä±ä³É·ĞÌÚÑªºìÄ§·ç,
	·²±»Æä´µ·÷Ö®Îï¾¡»Ó³ÉÆû, ÈÛÈË¿Ú±ÇÁîÈËÖÏÏ¢
	Õ£ÑÛ¼ä, ·ÙÌìÄ§Íõß³ºÈ»Øµ´"HIW"¡®³àÓü»ìãä Ä§µÀÕÚÌì¡¯"HIR", ÈÛÈÚÄ§ÆøĞ®×ÅÊ´¹Ç
