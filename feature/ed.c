...
#define LOGFILE	"MODIFICATION"		// 档案异动记录档

void start_edit(string filename);       // 呼叫编辑器，开始编辑
private void _append(object pl);        // 在游标后面加字
...
private void _join(object pl);                  // 将下行文字移到此行后面
...
J  Ctrl-P    将下行文字接到此行后面       O            在游标上方加一新行\r
        ...
        {        // 若有资料输入
                if (sscanf(str, "%s %s", _new, old) != 2)
                        return _message(pl, "输入格式错误。", "_message_done",
                                HIDE);
                me["keymap"][_new] = old;
                _message(pl, sprintf("好了，从此以后新定义键「%s”就有原功能键「%s”的功能了！",
