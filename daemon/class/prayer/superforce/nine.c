...
skill=this_player()->query("functions/nine/level");
        ...
        if( (string)me->query_skill_mapped("force")!= "superforce")
         return notify_fail("Ãµµ´Ï¼±ØĞëÊ¹ÓÃ»ëÌìĞÄ·¨²ÅÄÜ·¢»ÓÍşÁ¦¡£\n");
        if( target != me ) return notify_fail("ÄãÖ»ÄÜÓÃ»ëÌìĞÄ·¨ÌáÉıÄÚÁ¦ÍşÁ¦¡£\n");
        if (me->query_skill("superforce",1) < 30)
            return notify_fail("ÄãµÄ»ëÌìĞÄ·¨µÈ¼¶²»×ã£¬ĞëÒª 30 ¼¶¡£\n");
        if (me->query("force") < skill*5)
            return notify_fail("ÄãµÄÄÚÁ¦²»¹»£¬ÎŞ·¨ÁìÎòÃµµ´Ï¼µÄ»ëÌìÄÚ¾¢¡£\n");
        if( (int)me->query("max_force") < 300 )
        return notify_fail("ÄãµÄÄÚÁ¦¸ù»ù²»×ã,ÎŞ·¨ÍêÈ«·¢»Ó³öÃµµ´Ï¼ĞÛ»ëµÄÄÚ¾¢¡£\n");
        if(me->query_temp("nine")) return notify_fail("ÄãÒÑ¾­ÔÚÓÃÁË¡£\n");
        ...
        me->set_temp("nine",1);
        message_vision(
        HIC "$NÔËÆğ»ëÌìĞÄ·¨µÚ¶ş²ãµÄ"CYN"Ãµµ´Ï¼"HIC"£¬ÉíÉÏ·¢³öÈçÍíÏ¼°ãµÄ¹âÃ£ĞÎ³ÉÎŞÊıµÄÆøÁ÷!!\n" NOR,me);
...
void remove_effect(object me, int amount)
{
        ...
        me->add_temp("apply/dodge", -3);
        me->add_temp("apply/unarmed", -3);
        tell_object(me,"ÄãÉíÉÏµÄÃµµ´Ï¼Æø¾¢ÂıÂıµÄÏûÍËÖĞ,¹âÃ£Ò²ËæÖ®ÏûÉ¢¡£\n");
        if(skill < 50)
