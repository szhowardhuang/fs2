...
skill=this_player()->query("functions/bluesea/level");
        ...
        if( (string)me->query_skill_mapped("force")!= "superforce")
         return notify_fail("µå²×º£±ØĞëÊ¹ÓÃ»ëÌìĞÄ·¨²ÅÄÜ·¢»ÓÍşÁ¦¡£\n");
        if( target != me ) return notify_fail("ÄãÖ»ÄÜÓÃ»ëÌìĞÄ·¨ÌáÉıÄÚÁ¦ÍşÁ¦¡£\n");
        if (me->query_skill("superforce",1) < 110)
            return notify_fail("ÄãµÄ»ëÌìĞÄ·¨µÈ¼¶²»×ã£¬ĞëÒª 110 ¼¶¡£\n");
        if (me->query("force") < 350+skill*5)
            return notify_fail("ÄãµÄÄÚÁ¦²»¹»£¬ÎŞ·¨Ê¹ÓÃµå²×º£¡£\n");
        if( (int)me->query("max_force") < 1300 )
        return notify_fail("ÄãµÄÄÚÁ¦¸ù»ù²»×ã,ÎŞ·¨ÍêÈ«·¢»Ó³öµå²×º£ÅìÅÈµÄÄÚ¾¢¡£\n");
        if(me->query_temp("bluesea")) return notify_fail("ÄãÒÑ¾­ÔÚÓÃÁË¡£\n");
        ...
        message_vision(
        HIC "$NÔËÆğ»ëÌìĞÄ·¨µÚÆß²ã"HIB"[µå²×º£]"HIC"£¬ÕæÆøÁ÷´Ü£¬ÄºµØÖÜÎ§³öÏÖ"HIB"º£Ğ¥°ãµÄÄÚ¾¢"HIB"!!\n" NOR,me);
...
void remove_effect(object me, int amount)
{
        ...
        me->add_temp("apply/move", -20);
        me->add_temp("apply/dodge", -20);
        tell_object(me,"ÄãËùÉ¢·¢³öÀ´µÄ[35mµå²×º£[0mÆø¾¢Ù¿µØÏûÊ§ÎŞ×Ù¡£\n");
        if(skill<100)
