...
void create()
{
        ...
        "Æ®ÑôÉÈ"        : (: do_fan :),
        "¶Îö²"          : (: do_mime :),
        "Áè²¨Î¢²½"      : "ÊÇ¶Î¼ÒµÄÆæ²Å¡¸¶ÎÓş¡±Ñ§»áµÄÆæÃÅ²½·¨£¬ÎÒ¼ÇµÃËûÓĞÄÃÒ»±¾ÃØóÅ¸øÎÒ¡£",
        "Áè²¨Î¢²½ÃØóÅ"  : (: do_linpo :),
        ]));
        set("force",5000);
        set("max_force",5000);
...
string do_linpo()
{
        ...
        if(me->query("family/family_name")!="¶Î¼Ò")
          return "ÄãÎÊÌ«¶àÁË°É£¬Õâ²»¹ØÄãµÄÊÂ!!";
        if(me->query_skill("linpo-steps",1)!=100){
          command("slap "+me->query("id"));
          return "Äã»ù±¾µÄ¶¼»¹Ã»Ñ§ºÃ£¬¾ÍÔÚÏëÊ²Ã´ÃØóÅ???";
        }

        if(!me->query("quests/catch_thief"))
          tell_object(me,"¶ÎÔÆĞ¡ÉùµÄ¸æËßÄã:°¦£¬¼Ò³ó²»¿ÉÍâÑï£¬Äã»¹ÊÇ±ğÔÙÎÊÁË!!\n");
        else
          tell_object(me,"¶ÎÔÆÉúÆøµÄËµ:ºß£¬»¹²»ÊÇ±»ÕÔÇÙ·çÄÇ¸öÅÑÍ½¸øÍµ×ßÁË!!\n");
...
int accept_object(object me, object obj)
{
        ...
        else if(obj->query("name")=="Áè²¨Î¢²½¾íÖá" && obj->query("linpo_quest"))
        {
          if(me->query("marks/ask_linpo")!=3){
            write("¶ÎÔÆ¿´ÁË¿´ÊÖÉÏµÄ¶«Î÷£¬¾õµÃÒ»µãÓÃ´¦Ò²Ã»ÓĞ£¬ÓÚÊÇÔË¹¦°Ñ"+
                  obj->query("name")+"Äó¸ö·ÛËé!!\n");
            destruct(obj);
          }
          else{
            command("say Ã»´í£¬¾ÍÊÇÕâ±¾ÃØóÅ");
            command("think");
            command("hmm");
            command("say Èç¹ûÎÒÃ»¼Ç´íµÄ»°£¬Õâ¶«Î÷ÉÙÁË¼¸Ò³£¬Ç°ÃæËù¼ÇÔØµÄÕâĞ©ÎÒ¶¼ÒÑ¾­½Ì¹ıÄãÁË!!\n");
            command("give "+obj->query("id")+" to "+me->query("id"));
            me->set("marks/ask_linpo",4);
            command("say Èç¹ûÄãÕæµÄÏë°ÑÁè²¨Î¢²½µÄ¾«»ªÑ§»á£¬¾ÍÈ¥ÕÒ¶ÎÓş°É!!\n");
          }
        }
        else
        {
...
void die()
{
        ...
        j=random(6000);
          ...
          message_vision(HIM"\n´Ó¶ÎÔÆµÄÉíÉÏµôÏÂÁËÒ»¼şÆæ¹ÖµÄ¶«Î÷!!\n"NOR,winner);
          write_file("/log/sky/peace_stone",sprintf("%s(%s) ÈÃ¶ÎÔÆµôÏÂÁËºÍÆ½Ê¯ÓÚ %s\n",
