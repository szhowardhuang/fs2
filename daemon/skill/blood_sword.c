...
//11
              ([ "action"     :HIW"$N使出血魔禁断奥义"+HIM"----血雾刃---"NOR"
"+HIW"随着一阵阵的血雾$N以消失于阵阵血雾中，只见血雾渐渐包围$n"NOR,
...
void new_1(object me, object victim, object  weapon, int damage)
{
 ...
 else
 {
   for(i=1;i<=4;i++)
   {
     ...
     if( random(me->query("combat_exp")) >random(victim->query("combat_exp"))/6 )
    {
     message_vision(sprintf(HIB"强大魔杀如利刃般刺进$n体中，令$n痛不欲生！\n"NOR),me,victim);
...
{
  message_vision(HIW+HBBLU"封印在魔剑剑中的五道魂魄飞奔而出，以五魂连星化成一招极杀之招∶‘魂剑五魄’\n"NOR,me,victim);
  for(i=1;i<=5;i++)
  {
    switch(i)
    {
     case 1:
message_vision(sprintf(HIB"魔剑魂魄╠═ "HIC"蓝˙糜"HIB" ═╣以魔意为剑向$n劈出致命一击。\n"NOR),me,victim);
     break;
     case 2:
message_vision(sprintf(HIB"魔剑魂魄╠═ "HIR"红˙爓"HIB" ═╣以杀意为剑向$n劈出致命一击。\n"NOR),me,victim);
     break;
     case 3:
message_vision(sprintf(HIB"魔剑魂魄╠═ "HIC"青˙岚"HIB" ═╣以残意为剑向$n劈出致命一击。\n"NOR),me,victim);
     break;
     case 4:
message_vision(sprintf(HIB"魔剑魂魄╠═ "HIG"绿˙魅"HIB" ═╣以血意为剑向$n劈出致命一击。\n"NOR),me,victim);
     break;
     case 5:
message_vision(sprintf(HIB"魔剑魂魄╠═ "HIW"白˙魂"HIB" ═╣以恨意为剑向$n劈出致命一击。\n"NOR),me,victim);
     break;
    }
          if( random(me->query("combat_exp")) >random(victim->query("combat_exp"))/6 )
     {
...
// 第三skill 判断式 (me->query("swordskill/sword3")==1)
...
// 第五skill 判断式 (me->query("swordskill/sword5")==1)
// 无解迷无法用连击
void at_quick(object me, object victim, object  weapon, int damage)
 {
 if (me->query("swordskill/sword5")!=1)
 {
  message_vision(sprintf(HIR"片片血雾...引起无限杀机...\n"NOR),me,victim);
 ...
 if (!me->query_temp("strike"))
  {
    ...
    for(i=0;i<=time;i++)
   {
     message_vision(sprintf(HBRED"血雾连天 - 魔意无穷 - 阵阵邪气 - 层层杀劲\n"NOR),me,victim);
     victim->add("kee",-damage);
   COMBAT_D->do_attack(me, victim, me->query_temp("weapon"), TYPE_QUICK);
   }
