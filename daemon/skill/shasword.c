...
mapping query_action(object me, object *weapon)
{
 ...
 bellicosity = (int)(me->query("bellicosity",1));
        ...
        while(i--)
        {
if(skill_level >= 95 && (me->query("family/family_name")=="ÏÉ½£ÅÉ") && me->query("id")!="murofu" && me->query("name")!="Ä½Èİ¸´"&& me->query("force",1) > 200 && random(100) > 65)
                          {
        message_vision( sprintf(HIW"        
...
if(me->query("force_factor") >= 16 && me->query("family/family_name")!="ÏÉ½£ÅÉ")
return action[20];
        if (limit < 10)
...
/*ÏÂÃæÕâĞĞÒ»Ö±ÈÃnpcÃ»ĞÄÌø,Çëappo²éÃ÷Ô­Òò by bss
