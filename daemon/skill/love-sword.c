...
//以下是相思碎梦刀诀
//9
(["action" :HIC"$N右手执$w"HIC"，形如流水，使出"HIY"‘悠美如梦’"HIC"，刀光淡淡，\n"
               "如远山的望眉，夕照的依稀，刀光闪在$n的眸里，$n冷不防地受到猛烈的攻击！"NOR,
               "dodge" :           0,
               "parry" :           0,
               "damage":         150,
               "force" :         150,
...
//10
(["action" :HIC"$N右手反转，$w"HIC"自鞘中翻覆而握，使出"HIY"‘轻盈若诗’"HIC"，\n"
               "刀光像一首动人的诗，轻的像一阵微风，柔的像一抹月色，$n招架不及，\n"
               "着了一道严重的刀伤！"NOR,
              "dodge" :          10,
              "parry" :          10,
              "damage":         200,
...
//11
(["action" :HIC"$N随叹息而出刀，使出"HIY"‘梦里花落’"HIC"，悠远的刀光去势之疾，快逾电光，\n"+
               "闪电般斩向$n的腹中，$n脸上的表情，正如心肺被撕裂的痛楚！"NOR,
              "dodge" :          20,
              "parry" :          20,
              "damage":         220,
              "force" :         200,
...
//12
(["action" :HIC"$N想也不想，亮出了手上的$w"HIC"，使出"HIY"‘细雨黄昏’"HIC"，刀光惊艳般\n"
               "的亮起，如流星自长空划过，$n感到无可抗拒的震怖，跌入嗜血的刀网中！"NOR,
              "dodge" :          30,
              "parry" :          30,
              "damage":         240,
              "force" :         240,
...
//13
(["action" :HIC"$N突然冲了出去，刀光绰约，像一抹夕晖，一场细雨，惊艳一撇"HIY"‘飞花逐月’"HIC"，\n"+
               "$n看见了如彩色一般的梦，刀过处，黑里溅出厉红，带出了残酷的现实！"NOR,
              "dodge" :          40,
              "parry" :          40,
              "damage":         300,
              "force" :         250,
...
//14
(["action" :HIC"$N轻弹刀锋，震起仿似一种相见时喜悦的轻颤"HIY"‘秋风赶尘’"HIC"，一刀挥出像一道岁月的梦痕，\n"+
               "刀落时还带着些许美丽的风华，$n没有退路，\n无法闪躲，不能招架！"NOR,
              "dodge" :          30,
              "parry" :          30,
              "damage":         300,
...
//15
(["action" :HIC"$N拔刀作龙吟，比琴鸣筝响还动听， 带着微微的香气和呻吟，"HIY"‘相思若梦’"HIC"刀光纷飞，\n"+
               "似千道银雨，森寒的刀劲犹如一张天罗地网，$n的去路已被封锁、切断！"NOR,
              "dodge" :          20,
              "parry" :          20,
              "damage":         350,
              "force" :         400,
...
//16
(["action" :HIC"$N使出"HIY"‘欲断魂飞’"HIC"，$w"HIC"陡然而起，瞬然而止，刀光漾起一种凄美的颜色，\n"+
               "在晨曦的长空中，化作一道璨耀精虹，一缕\急风，并透了刀气与刀风，急取$n！"NOR,
             "dodge" :            0,
             "parry" :            0,
             "damage":          400,
             "force" :          500,
...
en_sk = random((int)enemy[i]->query("combat_exp",1)/200000) + (int)enemy[i]->query_skill("dodge",1)/5+ (int)enemy[i]->query_skill("parry",1)/5 + random(10);
    for(j=0; j<limit+4; j++) {
        ...
        message_vision(HIG"\n梦里闪过快刀，风里掠过长剑，刀剑联手，劲碎裂，人破灭．．\n" NOR, enemy[i]);
        COMBAT_D->report_status(enemy[i]);
        }
        else message_vision(HIB"\n$N明心见性，左闪快刀，右避长剑，有惊无险．．\n" NOR, enemy[i]);
       }
       }
     if( random(me->query("cps")) < 3 )
       {
        me->add("sen", -30);
        message_vision(HIM"\n$N突然旋舞出极凄艳的杀气，刀剑合壁攻势因而瓦解．．\n"NOR, me ,enemy[i]);
        me->start_busy(1);
       }
     return action[random(sizeof(action))];
   }
        //刀剑合壁,enable parry dream-blade,enable sword love-sword
...
int valid_learn(object me)
{
        ...
        if(me->query("get-2skill-s") != 1 )         //解love-sword的mark
        {
         ...
         if( (me->query("potential") - me->query("learned_points")) < 5 ) {
                tell_object(me, "你的潜能不到五点, 无法练此刀法。\n");
                return 0;
         }
         if( bell > 100 )
         {
        ...
        tell_object(me, HIY"王小石说道：学此剑法需扣潜能、精、气、神各五点。\n");
        me->add("gin", -5);
        me->add("kee", -5);
        me->add("sen", -5);
        me->add("potential", -5);
