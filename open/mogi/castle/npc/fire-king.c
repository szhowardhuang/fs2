...
if(str=="throw dark to king" || str=="throw dark to fire king"){
 write(HIY"·ÙÌìÄ§ÍõÀäĞ¦£º¡¸¸Ò¶ÔÎÒÓÃ¶¾???¡±È¥ËÀ°É!!\n"NOR);
 command("perform fireforce.gold-fire");
 who->start_busy(1);
 king->kill_ob(who);
 return 1;
...
void heart_beat()
{
 object *all;
	...
	count = random(100);
	j = random(3)+1;
// ¼ÓÉÏ»úÂÊÇå³ı°×ÁúÓ²¼×¹¦ÄÜ by swy
// ¼ÓÉÏĞÂµÄÉäÊÖ¹¦ÄÜÓ°Ïì by swy
if( 30 > random(100) ) {
for(i=0;i<sizeof(all);i++) {
...
message_vision(HIG"\n·ÙÌìÄ§Íõ·¢³öÒ»µÀÇ¿ÁÒÂÌ¹â$NÉíÉÏµÄ°×ÁúÄÜÁ¿ÏûÊ§ÁË¡£\n"NOR,all[i]);
}}
...
// ÁÆÉËÌ«¿äÕÅÁË Ã¿Ò»´ÎĞÄÌøÓĞ 50% »áÁÆÉË
// ÕæµÄ´ò²»ËÀ ÔÙ¼ÓÉÏÒ»²ãµÄÅĞ¶Ï
// 0.5x0.7 = 0.35 ²î²»¶à 35%    by swy
if(random(100)>30) {
