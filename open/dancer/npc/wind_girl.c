...
void create()
{
            set_name("风灵儿",({"wind girl","girl"}));
            set("long","她是镜月岛四天女之一个风灵使，其美貌自然不是一般凡间
所能相比较，传闻中其特技‘风嚎’威力十分强大!!\n");
            set("gender","女性");
            set("class","dancer");
            set("family/family_name","夜梦小筑");
            set("nickname","四天女");
	    set("title",HIY"风灵使"NOR);
...
void heart_beat()
{
	...
	i=sizeof(enemy);
        if( is_fighting() ){
	if(random(100)>70)
message_vision(HIY"风灵儿使出了本身绝学‘风嚎’，现场的风灵们受到了风灵儿的感招颳起了一阵大风!!\n"NOR,this_object());
	if(random(5)>2)
	{
    message_vision(HIY"你一个闪躲不及，整个人受到了风灵们的重击!!\n"NOR,this_object());
	for(j=0;j<i;j++)
	{ 
	if (!enemy[j]->is_fighting() && enemy[j]->is_character() && !enemy[j]->is_corpse() 
            && living(enemy[j]) && enemy[j]!=ob) 
	{
		message_vision(HIY"风灵们把你给吹走了!!\n"NOR,ob);
