...
void create()
{
            set_name("水瞳儿",({"water girl","girl"}));
            set("long","她是镜月岛四天女之一的水灵使，其美貌自然不是一般凡间
所能相比较，传闻中其特技‘水缠术’十分恐怖!!\n");
            set("gender","女性");
            set("class","dancer");
            set("family/family_name","夜梦小筑");
            set("nickname","四天女");
	    set("title",HIB"水灵使"NOR);
...
void heart_beat()
{
	...
	i=sizeof(enemy);
        if( is_fighting() ){
	if(random(100)>70)
message_vision(HIB"水瞳儿使出了本身绝学‘水缠术’，天空突然一阵大雨，所有的水珠往你身上猛扑!!\n"NOR,this_object());
	if(random(5)>2)
	{
    message_vision(HIR"你一个闪躲不及，整个人被包围在水中动弹不得!!\n"NOR,this_object());
	for(j=0;j<i;j++)
	{ 
