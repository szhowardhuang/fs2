...
void create()
{
set_name("ÉîÌï¹§×Ó",({"mj", "mj's server"}));
        set("long",@LONG
  ÉîÌï¹§×Ó(Mj Ð¡½ã)£¬Ëý×¨ÃÅ¿´Ì¨×ÓµÄ£¬ÓÐÎÊÌâÇëÎÊËý£º
        Âé½«£¬Íæ·¨£¬¹æÔò
LONG);
        set_weight(1);
        set("unit","ÕÅ");
...
int do_gon(string str,object me)
{
        ...
        if (Touched && NowPlayerId[NowPlayer]==me->query("id")){
           NO_HU=0;
           message_vision("\n$NÁ³ÉÏÂ¶³öÐ°¶ñµÄÐ¦ÈÝËµµÀ£º°µ¸Ühehe.....\n\n",me);
           who["Mj"]=do_delete_Mj(who["Mj"],str,1);
...
¡¡¡¡Ã¿ºú¾ÍÓÐµ×Ì¨£µ£¬ÔÚ¼ÓÉÏËùµÃµÄÌ¨¾ÍÊÇ×ÜÌ¨£¬ÒªÊÇ×ÔÃþ¾Í£ª£³£¬Ïàµ±ÓÚÎåÒ»µÄËã·¨¡£
...
int show_top(string str)
{
        ...
        show+=HBWHT+"¨\n";
        show+="¨€"+HBRED+"¨‡ Ó¢ÎÄÃû×Ö ©¦  ÖÐÎÄÃû×Ö  ©¦ÒÑÍæ¾ÖÊý©¦ ×ÜÕ½¼£ ©¦×ÔÃþ´ÎÊý©¦·ÅÇ¹´ÎÊý¨Š"+HBWHT+"¨€\n";
        if (str=="all"){
           for (i=0; i<sizeof(MjData); i+=6){///
               show+="¨€"+HBRED;
               show+=sprintf("¨‡%10s©¦%12s©¦%8s©¦%8s©¦%8s©¦%8s¨Š",
