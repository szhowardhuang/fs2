...
if(50>random(100)) {
      ...
      message_vision(RED"你因为不断击发九天狂雷, 大耗真力, 暂时无法动弹!\n"NOR,me,victim);
      me->start_busy(1);
}
}
...
void sp1(object me, object victim, object weapon, int damage)
{
      ...
      me->add("kee",-100);      //以下也将做同样限制,威力越大,相对付出也越多
      message_vision(NOR"\n",me,victim);
}
...
message_vision("              "HBGRN+HIW"$N拳含"HIR"‘中岳．嵩山’"HIW"严谨中正之态，向$n发劲!!"NOR"\n              ",me,victim);
      ...
      if(damage>=700)
      {
         ...
         me->add("force",-800);      //五岳合一威力超大,so自身伤害也最重
         me->add("kee",-300);
         me->start_busy(1);
         COMBAT_D->report_status(victim);
      }
      ...
      message_vision(RED"              此招耗去$N极大的体力, $N一时之间,只感到欲振乏力.."NOR"\n",me,victim);
      message_vision(NOR"\n",me,victim);
}
