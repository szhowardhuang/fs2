...
void create ()
{
  set ("short", "梦居");
  set ("long", @LONG
这是任正晴的寝室，摆设十分简单，只有一张床和一些简单家俱看
似单纯，却暗藏玄机，听说任正晴为了怕人趁他睡觉偷袭他，暗藏
了许多机关，没事别乱闯这里，免的死于非命！
LONG);
  set("exits", ([ /* sizeof() == 1 */
...
int do_move(string str)
{
        if(!str || str != "桌子")
                ...
                message_vision("$N移动了木桌，此时赫然发现木桌的右上桌脚出现了一个暗扣(button)。\n", this_player());
        }
        else {
                delete_temp("box_moved");
                message_vision("$N将木桌移回原位。\n", this_player());
                if(query("exits/down"))
                        delete("exits/down");
        }
        return 1;
